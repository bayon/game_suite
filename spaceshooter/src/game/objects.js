game.module('game.objects').body(function() {
	...
	game.createClass('Enemy', 'Entity', {
		...
		onUpdate : function() {
			// Remove when out of screen
			if (this.sprite.position.y - this.sprite.height / 2 > game.system.height / 4)
				this.remove();
		}
	});
	...
	game.createClass('Explosion', {
		init : function(x, y) {
			...
			this.sprite.anchor.set(0.5, 0.5);
			this.sprite.position.set(x, y), this.sprite.addTo(game.scene.mainContainer);
